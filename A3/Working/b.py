...
class MessageExchangeService(ca_pb2_grpc.MessageExchangeServiceServicer):
    def __init__(self, private_key, public_keyOfOtherClient):
...
def serve():
    ...
    ca_pb2_grpc.add_MessageExchangeServiceServicer_to_server(MessageExchangeService(b_private_key, client_b.publicKeyOfOtherClient), server)
    server.add_insecure_port('[::]:50053')
    server.start()
